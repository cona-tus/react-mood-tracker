...
'무드로그'는 감정 상태를 기록하는 웹 애플리케이션입니다. 정신 건강에 관심이 있거나, 스트레스를 관리 또는 감정 조절 능력을 향상시키고 싶은 사람들을 위해 제작되었습니다. 사용자는 기록하고 싶은 기간을 설정하고 해당 기간 동안 일기를 작성할 수 있습니다. 무드로그는 작성된 일기를 바탕으로 감정 수치를 시각화하고 통계화하여 제공합니다. 또한 반응형으로 디자인되어 다양한 디바이스 환경에서 언제 어디서든 편리하게 이용할 수 있습니다.
...
noteState의 전역 상태 관리에 어려움이 있었습니다. 처음에는 journalList가 필요한 컴포넌트에만 상태를 전달하려고 하여 notes와 journalList를 별도의 context로 분리했습니다.

그러나 journal을 업데이트할 때마다 notes까지 연쇄적으로 setState를 수행해야 했으며, 두 상태를 동시에 업데이트하는 과정이 불편하다고 느껴졌습니다.

따라서 이러한 문제를 해결하기 위해 noteState 객체를 하나의 context로 관리하는 방식으로 변경했습니다. 이를 통해 중복된 업데이트를 방지하고 상태 간의 일관성을 유지할 수 있었습니다.

또한 useReducer 훅과 immer를 통해 상태 업데이트 작업을 보다 간편하게 처리할 수 있었습니다.

### - 모바일 환경 렌더링

모바일 환경에서 Header의 Button이 렌더링되지 않는 문제가 발생했습니다. 이전 프로젝트에서도 유사한 문제를 경험했던 것이 떠올랐고, 해당 문제를 해결하기 위해 button 요소 내부에 span 요소를 생성하여 flex를 사용하여 중앙 정렬을 해주었습니다.

## 5-2. 후기

무드로그 프로젝트를 기획할 당시의 목표는 CRUD 작업을 통해 데이터를 관리하는 것이었습니다. 컨텍스트와 리듀서를 활용하여 전역 데이터를 가공하는 과정에서 데이터 관리에 대한 경험을 쌓을 수 있었습니다.

그러나 기능 구현에 주력하기 위해 최적화 작업을 하지 못했던 점이 아쉬웠습니다. 앞으로는 메모이제이션에 대해 공부하려고 합니다.

마지막으로, 일기 리스트를 작성순으로 정렬하거나 특정 감정만 필터링하는 기능을 추가한다면 더욱 완성도 있는 앱을 사용자에게 제공할 수 있을 것으로 생각됩니다.
